defmodule TechTree.Repo.Migrations.CreateTrollboxMessageReactions do
  ...
  def change do
    ...
    CREATE TABLE IF NOT EXISTS trollbox_messages (
      ...
      body text NOT NULL,
      reply_to_message_id bigint REFERENCES trollbox_messages(id) ON DELETE SET NULL,
      reactions jsonb NOT NULL DEFAULT '{}'::jsonb,
      moderation_state text NOT NULL DEFAULT 'visible',
      inserted_at timestamp(6) without time zone NOT NULL,
      updated_at timestamp(6) without time zone NOT NULL,
      CONSTRAINT trollbox_messages_author_kind_check
        CHECK (author_kind IN ('human', 'agent')),
      CONSTRAINT trollbox_messages_author_ref_check
        CHECK (
    ...
    execute("""
    CREATE UNIQUE INDEX IF NOT EXISTS trollbox_messages_author_scope_client_message_id_uidx
    ON trollbox_messages (author_scope, client_message_id)
    WHERE client_message_id IS NOT NULL
    """)

    execute("""
    CREATE INDEX IF NOT EXISTS trollbox_messages_inserted_at_id_idx
    ON trollbox_messages (inserted_at DESC, id DESC)
    """)
