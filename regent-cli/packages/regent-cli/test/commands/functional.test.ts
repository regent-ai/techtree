...
import { RegentRuntime, writeInitialConfig } from "@regent/runtime";
...
describeNetwork.sequential("CLI functional flows against the real runtime", () => {
  ...
  it("covers public reads, authenticated writes, and transport status through the CLI", async () => {
    const notebookPath = path.join(path.dirname(configPath), "publish.py");
    fs.writeFileSync(notebookPath, "print('cli functional publish')\n", "utf8");
    ...
    const createOutput = await captureOutput(async () =>
      runCliEntrypoint([
        "techtree",
        "node",
        "create",
        ...
        "2:supports",
        "--idempotency-key",
        "cli-functional-node-key",
      ]),
    );

    expect(createOutput.result).toBe(0);
    const createdNode = JSON.parse(createOutput.stdout) as { data: { node_id: number; anchor_status: string } };
    expect(createdNode.data.anchor_status).toBe("pending");

    const getOutput = await captureOutput(async () =>
      runCliEntrypoint([
        "techtree",
        "node",
        "get",
        String(createdNode.data.node_id),
        "--config",
        configPath,
      ]),
    );
    expect(getOutput.result).toBe(0);
    expect(JSON.parse(getOutput.stdout)).toEqual({
      data: expect.objectContaining({
        id: createdNode.data.node_id,
        title: "CLI functional publish",
        sidelinks: [
          expect.objectContaining({
            dst_node_id: 2,
            tag: "supports",
    ...
    const commentOutput = await captureOutput(async () =>
      runCliEntrypoint([
        "techtree",
        "comment",
        "add",
        ...
        String(createdNode.data.node_id),
        "--body-markdown",
        "CLI functional comment",
        "--idempotency-key",
        "cli-functional-comment-key",
      ]),
    );
    expect(commentOutput.result).toBe(0);

    const watchOutput = await captureOutput(async () =>
      runCliEntrypoint(["techtree", "watch", String(createdNode.data.node_id), "--config", configPath]),
    );
    expect(watchOutput.result).toBe(0);
    expect(JSON.parse(watchOutput.stdout)).toEqual({
      data: expect.objectContaining({
        node_id: createdNode.data.node_id,
        watcher_type: "agent",
      }),
    });

    const unwatchOutput = await captureOutput(async () =>
      runCliEntrypoint(["techtree", "unwatch", String(createdNode.data.node_id), "--config", configPath]),
    );
    ...
    expect(JSON.parse(commentsOutput.stdout)).toEqual({
      data: expect.arrayContaining([
        expect.objectContaining({
          ...
          node_id: createdNode.data.node_id,
          body_markdown: "CLI functional comment",
        }),
      ]),
    });
    ...
    const starOutput = await captureOutput(async () =>
      runCliEntrypoint(["techtree", "star", "1", "--config", configPath]),
    );
    ...
    const unstarOutput = await captureOutput(async () =>
      runCliEntrypoint(["techtree", "unstar", "1", "--config", configPath]),
    );
  ...
  it("returns deterministic local failure paths for mutating commands", async () => {
    const missingNotebookOutput = await captureOutput(async () =>
      runCliEntrypoint([
        "techtree",
        "node",
        "create",
    ...
    expect(missingNotebookOutput.result).toBe(1);
    expect(JSON.parse(missingNotebookOutput.stderr)).toEqual({
      error: {
        message: expect.stringContaining("missing-notebook.py"),
      },
    });

    const unauthenticatedCommentOutput = await captureOutput(async () =>
      runCliEntrypoint([
        "techtree",
        "comment",
        "add",
    ...
    expect(unauthenticatedCommentOutput.result).toBe(1);
    expect(JSON.parse(unauthenticatedCommentOutput.stderr)).toEqual({
      error: {
  ...
  it("runs doctor in human, json, scoped, and full modes through the CLI", async () => {
    ...
    expect(humanDoctor.stdout).toContain("Regent Doctor");
    expect(humanDoctor.stdout).toContain("[ok] techtree health reachable");
