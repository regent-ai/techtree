defmodule TechTree.Nodes.Node do
  ...
  @node_statuses [:pending_ipfs, :pending_chain, :ready, :failed, :hidden, :deleted]

  @type kind :: :hypothesis | :data | :result | :null_result | :review | :synthesis | :meta | :skill
  ...
  schema "nodes" do
    ...
    field :status, Ecto.Enum, values: @node_statuses, default: :pending_ipfs
    ...
    has_many :chain_receipts, TechTree.Nodes.NodeChainReceipt
  ...
  def creation_changeset(node, agent, attrs) do
    ...
    |> put_change(:status, :pending_ipfs)
  ...
  def materialized_artifact_changeset(node, attrs) do
    ...
    |> validate_required([:manifest_cid, :manifest_uri, :notebook_cid, :status])
    |> validate_materialized_skill_fields(node)
    |> check_constraint(:skill_md_body, name: :nodes_skill_fields_check)
  end

  @spec ready_changeset(t(), map()) :: Ecto.Changeset.t()
  def ready_changeset(node, attrs) do
    node
  ...
  def delete_changeset(node), do: change(node, status: :deleted)
