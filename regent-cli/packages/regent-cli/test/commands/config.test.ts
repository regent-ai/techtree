...
describe("config commands", () => {
  it("prints the normalized effective config", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "regent-cli-config-read-"));
  ...
  it("writes a full validated replacement config from @file input", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "regent-cli-config-write-"));
    const configPath = path.join(tempDir, "config.json");
    ...
    const printed = parsePrintedJson<{
      ok: boolean;
      configPath: string;
      config: { techtree: { baseUrl: string; defaultChainId: number }; runtime: { logLevel: string } };
    }>(stdout);
