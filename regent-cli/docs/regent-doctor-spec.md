...
- `regent doctor runtime|auth|techtree|transports` runs only the named scope.

## 4. Architecture placement

The diagnostic logic belongs in `packages/regent-runtime`, not `packages/regent-cli`.
...
interface DoctorRunScopedParams extends DoctorRunParams {
  scope: "runtime" | "auth" | "techtree" | "transports";
}
...
`packages/regent-runtime/test/doctor/*.test.ts`
