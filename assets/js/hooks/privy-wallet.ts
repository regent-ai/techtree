...
export interface PrivyLike {
  ...
  auth: {
    ...
    siwe: {
      ...
      loginWithSiwe(signature: string, wallet?: PrivyWalletShape, message?: string): Promise<unknown>
    }
  }
}
...
export function walletForUser(user: PrivyUser): string | null {
  ...
  if (typeof user.wallet?.address === "string" && user.wallet.address.trim().length > 0) {
    return user.wallet.address.trim()
  }
  ...
  return linkedAccount?.address?.trim() ?? null
}
...
export async function requireEthereumProvider(): Promise<EthereumProvider> {
  ...
  if (!provider || typeof provider.request !== "function") {
    throw new Error("Open a browser wallet before you continue.")
  }
...
export async function buildPrivyWallet(provider: EthereumProvider): Promise<PrivyWalletShape> {
  ...
  if (!chainId) {
    throw new Error("The connected wallet did not report a usable chain.")
  }
...
export async function signWithConnectedWallet(
  ...
  if (
    ...
    throw new Error("Finish this step with the same wallet you connected.")
  }
