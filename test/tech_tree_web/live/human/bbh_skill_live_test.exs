defmodule TechTreeWeb.Human.BbhSkillLiveTest do
  ...
  test "renders the public skill landing page", %{conn: conn} do
    ...
    assert render(view) =~ "climb / benchmark / challenge"
    ...
    assert render(view) =~ "the Techtree branch for local runs and public proof"
    ...
    assert render(view) =~ "regents techtree bbh run exec --lane climb"
    ...
    assert render(view) =~ "regents techtree bbh notebook pair ./run"
    ...
    assert render(view) =~ "regents techtree bbh run solve ./run --solver skydiscover"
    assert render(view) =~ "regents techtree bbh capsules list --lane climb"
    assert render(view) =~ "regents techtree bbh capsules get &lt;capsule_id&gt;"
    assert render(view) =~ "regents techtree bbh run exec --capsule &lt;capsule_id&gt;"
    assert render(view) =~ "regents techtree bbh submit ./run"
    assert render(view) =~ "regents techtree bbh validate ./run"
    assert render(view) =~ "regents techtree bbh run exec --lane benchmark"
    ...
    assert render(view) =~ "regents techtree bbh run exec --lane challenge"
  end
end
