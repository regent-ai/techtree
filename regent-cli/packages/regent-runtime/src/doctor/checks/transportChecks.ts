      ...
      run: async (ctx) => {
        ...
        return {
          ...
          message: fs.existsSync(policyPath)
            ? "XMTP public policy file is present"
            : "XMTP public policy file is missing; run `regent xmtp policy init`",
          details: {
            path: policyPath,
          },
          remediation: fs.existsSync(policyPath) ? undefined : "Run `regent xmtp policy init`",
        };
      },
    },
    {
      ...
      run: async (ctx) => {
        ...
        return {
          ...
          message: ready
            ...
            : "XMTP key material is missing; run `regent xmtp init`",
          details: {
            walletKeyPath: ctx.config.xmtp.walletKeyPath,
            dbEncryptionKeyPath: ctx.config.xmtp.dbEncryptionKeyPath,
            walletExists,
          ...
          remediation: ready ? undefined : "Run `regent xmtp init`",
        };
      },
    },
    {
