defmodule TechTree.XMTPMirror do
  ...
  def list_public_messages(attrs \\ %{}) when is_map(attrs) do
    ...
    if room do
      ...
      |> limit(^QueryHelpers.parse_limit(attrs, @default_limit))
      |> Repo.all()
    else
      []
    end
  ...
  defp enqueue_expired_presence_evictions(%XmtpRoom{} = room, now) do
    ...
    Enum.reduce(expired_presences, 0, fn presence, count ->
      case presence.evicted_at do
        nil ->
          _ =
            create_eviction_command(presence, room)
  ...
  defp membership_state_for(%HumanUser{} = human, %XmtpRoom{} = room) do
    ...
    case latest do
      ...
      %XmtpMembershipCommand{op: "add_member", status: "pending"} ->
        "join_pending"

      %XmtpMembershipCommand{op: "add_member", status: "processing"} ->
        "join_pending"
      ...
      %XmtpMembershipCommand{op: "remove_member", status: "pending"} ->
        "leave_pending"

      %XmtpMembershipCommand{op: "remove_member", status: "processing"} ->
        "leave_pending"
  ...
  defp resolve_room(room_key) when is_binary(room_key) do
    get_room_by_key(room_key)
  end

  defp resolve_room(room_id) when is_integer(room_id) do
    Repo.get(XmtpRoom, room_id)
  end
