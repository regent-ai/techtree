...
import * as RegentRuntime from "@regent/runtime";
...
export async function runConfigWrite(args: ParsedCliArgs): Promise<void> {
  ...
  const writeConfigReplacement = (RegentRuntime as Record<string, unknown>).writeConfigReplacement as (
    configPath: string,
    nextConfig: unknown,
  ) => unknown;
  const writtenConfig = writeConfigReplacement(configPath, parsedInput);
