...
const requireSessionJson = (value: Record<string, unknown>): void => {
  printJson(value);
};
...
export async function runAutolaunchBidsExit(args: ParsedCliArgs): Promise<void> {
  ...
  const body = { tx_hash: requireArg(getFlag(args, "tx-hash"), "tx-hash") };

  printJson(
    await requestJson("POST", `/api/bids/${encodeURIComponent(bidId)}/exit`, {
      body,
      requireSession: true,
    }),
  );
...
export async function runAutolaunchBidsClaim(args: ParsedCliArgs): Promise<void> {
  ...
  printJson(
    await requestJson("POST", `/api/bids/${encodeURIComponent(bidId)}/claim`, {
      body,
      requireSession: true,
    }),
  );
