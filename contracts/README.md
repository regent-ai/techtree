...
- `Techtree` contract scope here is the registry, TECH rewards, emissions, exit-fee splitter, and paid-node settlement surfaces
...
- the TECH v0.2 token, locked agent reward vault, reward router, leaderboard registry, emission controller, and exit-fee splitter
...
- `src/TechExitFeeUsdcSplitter.sol`
...
export TECH_USDC_TOKEN=...
export TECH_REGENT_REVENUE_STAKING=...
...
export WETH_USDC_POOL_FEE=...
export WETH_USDC_POOL_TICK_SPACING=...
export TECH_WETH_MIN_LIQUIDITY=...
export WETH_USDC_MIN_LIQUIDITY=...
...
cast code "$TECH_EXIT_FEE_SPLITTER_ADDRESS" --rpc-url "$BASE_MAINNET_RPC_URL"
```
